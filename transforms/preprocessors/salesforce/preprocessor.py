...
from software_defined_integrations.transforms.preprocessors.salesforce.transforms.fields import (
    generate_fields_metadata,
)
from software_defined_integrations.transforms.preprocessors.salesforce.transforms.links import (
    generate_links_metadata,
)
from software_defined_integrations.transforms.preprocessors.salesforce.transforms.objects import (
...
class Salesforce(Preprocessor):
    ...
    def transform(
        ...
        fields_metadata_df = generate_fields_metadata(
            dd_tables["sys_tablecolumns"].dataframe()
        )
        links_metadata_df = generate_links_metadata(
            dd_tables["sys_foreignkeys"].dataframe()
        )

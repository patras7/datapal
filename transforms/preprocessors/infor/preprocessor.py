...
from software_defined_integrations.transforms.preprocessors.infor.transforms.fields import (
    generate_fields_metadata,
)
from software_defined_integrations.transforms.preprocessors.infor.transforms.links import (
    generate_links_metadata,
)
from software_defined_integrations.transforms.preprocessors.infor.transforms.objects import (
    generate_objects_metadata,
)


@register(SourceType.INFOR)
class Infor(Preprocessor):
    def __init__(self, pipeline_config: PipelineConfig, source_config: SourceConfig):
        super().__init__(pipeline_config, source_config)

    def transform(
        self, ctx: TransformContext, **dd_tables: TransformInput
    ) -> Tuple[DataFrame, DataFrame, DataFrame]:
        objects_metadata_df = generate_objects_metadata(
            ctx, dd_tables["infor_metadata_raw_json"]
        )
        fields_metadata_df = generate_fields_metadata(
            ctx, dd_tables["infor_metadata_raw_json"]
        )
        links_metadata_df = generate_links_metadata(
            objects_metadata_df, fields_metadata_df
        )
